    ...
    if dvd_num == 0.0 and dvr_num == 0.0 and dresses_num == 0.0 and mobile_num == 0.0:
        ...
        def main_html():
            ...
            def write_to_html(one,two,three):
                ...
                write_to = """\n<tr>\n<td>
<h5>{}</h5><br>
<img src = '{}' alt= 'image missing' id= 'respond_image'><br>
<p>The cost: ${}</p>\n</td>\n</tr>""".format(one,two,three)
            ...
            if dvd_num > 0.0:
                for dvd in range(int(dvd_num)):
                    #Plate up into HTML
                    dvd_stuff = dvd_title_img_price[dvd]
                    price_change(dvd_stuff[2])
                    write_to_html(dvd_stuff[0], dvd_stuff[1], aud_dollar)
            ...
            #Get the scraped data and format to into the HTML code
            if dvr_num > 0.0:
                for dvr in range(int(dvr_num)):
                    #Plate up into HTML
                    dvr_stuff = dvr_title_img_price[dvr]
                    price_change(dvr_stuff[2])
                    write_to_html(dvr_stuff[0], dvr_stuff[1], aud_dollar)
                    #Put into list for the database
            ...
            #Get the scraped data and format to into the HTML code
            if dresses_num > 0.0:
                for dresses in range(int(dresses_num)):
                    #Plat eup into HTML
                    dresses_stuff = dresses_title_img_price[dresses]
            ...
            #Get the scraped data and format to into the HTML code
            if mobile_num > 0.0:
                for mobile in range(int(mobile_num)):
                    #Plate up into HTML
                    mobile_stuff = mobile_title_img_price[mobile]
...
</center>""".format(url_title,url_dvd,url_dvr,url_dresses,url_mobile))
